...
const Outstanding = () => {
  ...
  const fetchOutstanding = async () => {
    ...
    try {
      ...
      setError(
        "Failed to fetch outstanding transactions. Please try again later."
      );
  ...
  const filteredTransactions = outstandingTransactions.filter((transaction) => {
    const isStatusMatch =
      ...
      (filterStatus === "Partial" && transaction.status === "Partial");
    const isSearchMatch =
  ...
  const submitPayment = async () => {
    ...
    try {
      ...
      const response = await axios.patch(
        `/api/outstanding/${selectedTransaction.id}`,
        {
              ...
              {filteredTransactions.length > 0 ? (
                filteredTransactions.map((transaction) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {formatCurrency(transaction.final_outstanding_amount)}
                    </td>
                    <td className="px-4 py-3">
                      <button
                        onClick={() => {
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${getStatusClass(transaction.status, transaction.date)}`}
                      >
                        {transaction.status}
                      </span>
                    </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-96">
            ...
            <p className="mb-2">
              Pending Amount:{" "}
              {formatCurrency(selectedTransaction.final_outstanding_amount)}
            </p>
            <label className="block mb-1 font-medium" htmlFor="paymentAmount">
              Payment Amount
            </label>
            <input
              id="paymentAmount"
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-96 max-h-[80vh] overflow-y-auto">
            ...
            <p className="mb-2 font-medium">
              Customer: {detailsTransaction.customer_name}
            </p>
            <p className="mb-2">
              Total Amount: {formatCurrency(detailsTransaction.total_amount)}
            </p>
            <p className="mb-2">
              Pending Amount:{" "}
              {formatCurrency(detailsTransaction.final_outstanding_amount)}
            </p>
            <p className="mb-2">
              Paid Amount: {formatCurrency(detailsTransaction.paid_amount)}
            </p>
            {detailsTransaction.payment_history &&
            detailsTransaction.payment_history.length > 0 ? (
