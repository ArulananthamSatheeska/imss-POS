    ...
    useEffect(() => {
        ...
        const fetchCompanyDetails = async () => {
            try {
                ...
                const listResponse = await axios.get("http://localhost:8000/api/companies?fields=id,company_name");
                const companies = listResponse.data;
                if (companies.length > 0) {
                    // Fetch details of the first company
                    const firstCompanyName = companies[0].company_name;
                    const detailResponse = await axios.get(`http://localhost:8000/api/companies/${firstCompanyName}`);
