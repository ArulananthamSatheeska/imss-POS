...
const SalesReport = () => {
  ...
  const handleGenerateInvoiceApiCall = async (newInvoiceData) => {
    ...
    try {
      // Adjust payload keys to match backend expectations if needed
      const payload = {
        invoice: {
          no: newInvoiceData.invoice.no,
          date: newInvoiceData.invoice.date,
          time: newInvoiceData.invoice.time,
        },
        customer: {
          name: newInvoiceData.customer.name,
          address: newInvoiceData.customer.address,
          phone: newInvoiceData.customer.phone,
          email: newInvoiceData.customer.email,
        },
        purchaseDetails: {
          method: newInvoiceData.purchaseDetails.method,
          amount: newInvoiceData.purchaseDetails.amount,
        },
  ...
  const handleUpdateInvoiceApiCall = async (updatedInvoiceData, invoiceId) => {
    ...
    try {
      ...
      const payload = {
        invoice: {
          no: updatedInvoiceData.invoice.no,
          date: updatedInvoiceData.invoice.date,
          time: updatedInvoiceData.invoice.time,
        },
        customer: {
          name: updatedInvoiceData.customer.name,
          address: updatedInvoiceData.customer.address,
          phone: updatedInvoiceData.customer.phone,
          email: updatedInvoiceData.customer.email,
        },
        purchaseDetails: {
          method: updatedInvoiceData.purchaseDetails.method,
          amount: updatedInvoiceData.purchaseDetails.amount,
        },
        items: updatedInvoiceData.items.map(item => ({
