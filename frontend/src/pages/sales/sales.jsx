...
const SalesReport = () => {
  ...
  const fetchReportData = useCallback(async () => {
    ...
    try {
      ...
      if (invoiceResponse.data && Array.isArray(invoiceResponse.data.data)) {
        ...
        console.warn("Received invoice data is not in expected format:", invoiceResponse.data);
      ...
      if (salesResponse.data && Array.isArray(salesResponse.data.data)) {
        ...
        console.warn("Received sales data is not in expected format:", salesResponse.data);
      ...
      const combinedData = [...processedInvoices, ...processedSales].sort((a, b) => 
  ...
  const handleGenerateInvoiceApiCall = async (newInvoiceData, type = 'invoice') => {
    ...
    try {
      ...
      console.log(`API Success (${type === 'invoice' ? 'Invoice' : 'Sale'} Creation):`, response.data);
      fetchReportData();
  ...
  const handleUpdateInvoiceApiCall = async (updatedInvoiceData, id, type = 'invoice') => {
    if (!id) {
    ...
    try {
      ...
      console.log(`API Success (${type === 'invoice' ? 'Invoice' : 'Sale'} Update):`, response.data);
      fetchReportData();
  ...
  const handleDeleteSale = async (id, type = 'invoice') => {
    ...
    if (window.confirm(`Are you sure you want to delete ${type} ID: ${id}? This action cannot be undone.`)) {
      setLoading(true);
      const url = `${API_BASE_URL}/${type === 'invoice' ? 'invoices' : 'sales'}/${id}`;
      try {
        ...
        alert(`${type === 'invoice' ? 'Invoice' : 'Sale'} ID: ${id} deleted successfully.`);
        fetchReportData();
      } catch (error) {
        console.error(`Error deleting ${type} ${id}:`, error.response || error);
        alert(`Failed to delete ${type} ${id}. ${error.response?.data?.message || error.message}`);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleViewInvoice = (row) => {
    ...
    if (!row.customer_name) {
      row.customer_name = row.type === 'sale' ? "Walk-in Customer" : "Unknown Customer";
    }

    if (row.type === 'sale') {
      const products = (Array.isArray(row.items) ? row.items : []).map((item, index) => ({
        ...
        discount: parseFloat(item.discount) || 0,
        total: parseFloat(item.total) || (parseFloat(item.unit_price) || 0) * (parseFloat(item.quantity) || 1),
        serialNumber: index + 1,
      }));

      const totals = {
        totalQty: products.reduce((sum, p) => sum + (p.qty || 0), 0),
        subTotalMRP: products.reduce((sum, p) => sum + (p.mrp || 0) * (p.qty || 0), 0),
        totalItemDiscounts: products.reduce((sum, p) => sum + (p.discount || 0) * (p.qty || 0), 0),
        totalBillDiscount: parseFloat(row.discount) || 0,
        finalTotalDiscount: (products.reduce((sum, p) => sum + (p.discount || 0) * (p.qty || 0), 0)) + (parseFloat(row.discount) || 0),
      ...
      setInvoiceDataForPreview({
        ...
        items: (Array.isArray(row.items) ? row.items : []).map(item => ({
          id: item.id || null,
          description: row.type === 'sale' ? (item.product_name || "N/A") : (item.description || "N/A"),
          quantity: item.quantity || item.qty || 0,
          unit_price: item.unit_price || item.sales_price || 0,
          discountAmount: item.discount || item.discount_amount || 0,
          discountPercentage: item.discount_percentage || 0,
          total: item.total || (item.quantity * (item.unit_price || item.sales_price)) || 0,
    ...
    if (row.type === 'sale') {
      ...
      const mappedDataForEdit = {
        ...
        customer: {
          ...
          name: row.customer_name || (row.type === 'sale' ? "Walk-in Customer" : "Unknown Customer"),
        ...
        items: (Array.isArray(row.items) ? row.items : []).map((item) => {
          ...
          return {
            ...
            description: row.type === 'sale' ? (item.product_name || "") : (item.description || ""),
            qty,
            unitPrice,
            salesPrice: unitPrice,
            buyingCost: parseFloat(item.buying_cost || (item.total_buying_cost / qty) || 0),
            discountAmount: parseFloat(item.discount || item.discount_amount || 0),
      ...
      console.log('Mapped Data for Edit (Invoice):', JSON.stringify(mappedDataForEdit, null, 2));
    ...
    <div className="flex flex-col min-h-screen p-4 bg-transparent">
      ...
      {showEditInvoiceModal && invoiceToEdit && (
        <SalesInvoice
          ...
          onUpdateInvoice={(data, id) => handleUpdateInvoiceApiCall(data, id, invoiceToEdit.type)}
          onCancel={() => {
            setShowEditInvoiceModal(false);
            setInvoiceToEdit(null);
          }}
      ...
      {showEditSaleModal && saleToEdit && (
        <POSForm
          ...
          onSubmit={(formData) => handleUpdateSale({ ...formData, id: saleToEdit.id })}
      ...
      <div className="grid grid-cols-1 gap-4 mb-6 sm:grid-cols-2 lg:grid-cols-4">
        <div className="p-4 bg-white border-l-4 border-blue-500 rounded-lg shadow-md dark:bg-slate-800 dark:border-blue-400">
          <h3 className="mb-1 text-sm font-medium text-gray-600 dark:text-gray-400">Total Entries</h3>
        ...
        <div className="p-4 bg-white border-l-4 border-green-500 rounded-lg shadow-md dark:bg-slate-800 dark:border-green-400">
          <h3 className="mb-1 text-sm font-medium text-gray-600 dark:text-gray-400">Total Revenue</h3>
            ...
            <table className="min-w-full text-sm divide-y divide-gray-200 dark:divide-slate-600">
              <thead className="text-xs tracking-wider text-gray-700 uppercase bg-gray-100 dark:bg-slate-700 dark:text-gray-300">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold text-left whitespace-nowrap">Bill/Inv #</th>
                ...
                {filteredData.length === 0 && !loading ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-10 text-center text-gray-500 dark:text-gray-400">
                      No sales or invoice data found for the selected period or filters.
                    </td>
                  </tr>
                ) : (
                    ...
                    <React.Fragment key={`${row.type}-${row.id}`}>
                      ...
                      >
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="font-semibold text-gray-900 dark:text-gray-100">{row.customer_name}</div>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <span className={`px-2 py-1 inline-flex text-xs font-semibold leading-tight rounded-full
                            ${row.payment_method.toLowerCase() === 'cash' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' :
                              ...
                              row.payment_method.toLowerCase().includes('online') ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300' :
                              row.payment_method.toLowerCase().includes('cheque') ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300' :
                              row.payment_method.toLowerCase().includes('credit') ? 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300' :
                        ...
                        <td className="px-4 py-3 text-right whitespace-nowrap">
                          <div className="flex items-center justify-end gap-x-3">
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); handleDeleteSale(row.id, row.type); }}
                          ...
                          <td colSpan={7} className="px-4 py-4 md:px-6 md:py-4">
                            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                              <div className="p-3 border border-gray-200 rounded-md dark:border-slate-700">
                                <h4 className="mb-2 text-xs font-semibold tracking-wide text-gray-600 uppercase dark:text-gray-400">{row.type === 'invoice' ? 'Invoice' : 'Sale'} Summary</h4>
                                <div className="grid grid-cols-2 text-sm gap-x-4 gap-y-1">
                                  <div className="text-gray-500 dark:text-gray-400">Subtotal:</div>
                                  <div className="font-medium text-right text-gray-900 dark:text-white">{formatCurrency(row.subtotal)}</div>
                                  <div className="text-gray-500 dark:text-gray-400">Tax:</div>
                                  <div className="font-medium text-right text-gray-900 dark:text-white">+{formatCurrency(row.tax_amount)}</div>
                                  <div className="text-gray-500 dark:text-gray-400">Discount:</div>
                                  <div className="font-medium text-right text-red-600 dark:text-red-400">-{formatCurrency(row.discount)}</div>
                                  <div className="text-gray-500 dark:text-gray-400">Total:</div>
                                  <div className="font-bold text-right text-gray-900 dark:text-white">{formatCurrency(row.total_amount)}</div>
                                  ...
                                  <div className="font-medium text-right text-green-600 dark:text-green-400">{formatCurrency(row.purchase_amount)}</div>
                                  ...
                                  <div className="font-medium text-right text-gray-900 dark:text-white">{row.payment_method}</div>
                                  <div className="text-gray-500 dark:text-gray-400">Status:</div>
                                  <div className="font-medium text-right text-gray-900 capitalize dark:text-white">{row.status}</div>
                                </div>
                              </div>
                                        ...
                                        {row.items.map((item, i) => (
                                          <tr key={item.id || i}>
                                            <td className="px-2 py-1 font-medium text-gray-900 dark:text-white">
                                              {row.type === 'sale' ? (item.product_name || 'Unknown Product') : (item.description || 'N/A')}
                                            </td>
