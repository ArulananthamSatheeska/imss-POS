...
const DiscountScheam = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [productResponse, categoryResponse, customerResponse, schemeResponse] = await Promise.all([
          ...
          axios.get('http://127.0.0.1:8080/api/customers'), // Fetch customers
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (editSchemeId) {
        response = await axios.put(`http://127.0.0.1:8080/api/discount-schemes/${editSchemeId}`, payload);
        const updatedScheme = {
          ...response.data.data,
        ...
        response = await axios.post('http://127.0.0.1:8080/api/discount-schemes', payload);
        const newScheme = {
          ...response.data.data,
          appliesTo: response.data.data.applies_to,
          startDate: response.data.data.start_date,
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`http://127.0.0.1:8080/api/discount-schemes/${schemeToDelete}`);
