...
export const SupplierWiseProfit = () => {
    ...
    try {
      ...
      if (response.data?.reportData) {
        ...
        if (processedData.length === 0) {
          setError(
            `No data found for supplier "${selectedSupplier.label}"${fromDate && toDate ? ` between ${fromDate} and ${toDate}` : ""}.`
          );
        }
      } else {
        setReportData([]);
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `Supplier_Wise_Profit_Report${fromDate && toDate ? `_${fromDate}_to_${toDate}` : ""}.xlsx`
    );
  };
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text(
      `Supplier Wise Profit Report${fromDate && toDate ? ` (${fromDate} to ${toDate})` : ""}`,
      10,
      10
    );
    doc.autoTable({
    ...
    doc.save(
      `Supplier_Wise_Profit_Report${fromDate && toDate ? `_${fromDate}_to_${toDate}` : ""}.pdf`
    );
  };
