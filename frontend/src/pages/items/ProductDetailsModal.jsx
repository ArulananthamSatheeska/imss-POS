...
const ProductDetailsModal = ({ productId, onClose }) => {
  ...
  const wholesaleProfit = product.wholesale_price
    ? product.wholesale_price - product.buying_cost
    : 0;
  ...
  const fieldGroups = [
    ...
    },
    {
      ...
      fields: [
        {
          label: "Buying Cost",
          value: `LKR ${product.buying_cost?.toLocaleString()}`,
        },
        {
          label: "Selling Price",
          value: `LKR ${product.sales_price?.toLocaleString()}`,
        },
        { label: "MRP", value: `LKR ${product.mrp?.toLocaleString()}` },
        {
          label: "Minimum Price",
          value: `LKR ${product.minimum_price?.toLocaleString()}`,
        },
        {
          label: "Wholesale Price",
          value: product.wholesale_price
            ? `LKR ${product.wholesale_price?.toLocaleString()}`
            : "N/A",
        },
        {
          label: "Profit per Unit",
          value: `LKR ${profitPerUnit?.toFixed(2)} (${profitMargin?.toFixed(2)}%)`,
        },
      ],
    },
    {
      ...
      fields: [
        ...
        {
          label: "Opening Value",
          value: `LKR ${stockValue?.toLocaleString()}`,
        },
        {
          label: "Potential Revenue",
          value: `LKR ${potentialRevenue?.toLocaleString()}`,
        },
        {
          label: "Minimum Stock Qty",
          value: product.minimum_stock_quantity || "N/A",
        },
            ...
            <div className="mt-4 grid grid-cols-3 gap-2">
              <div className="bg-blue-50 dark:bg-blue-900/30 p-2 rounded-lg text-center border border-blue-100 dark:border-blue-900/50">
                ...
                <div className="font-bold text-blue-800 dark:text-blue-200">
                  LKR {product.buying_cost?.toLocaleString()}
                </div>
              </div>
              <div className="bg-green-50 dark:bg-green-900/30 p-2 rounded-lg text-center border border-green-100 dark:border-green-900/50">
                ...
                <div className="font-bold text-green-800 dark:text-green-200">
                  LKR {product.sales_price?.toLocaleString()}
                </div>
              </div>
