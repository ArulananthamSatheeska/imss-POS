...
const PurchaseInvoice = () => {
  ...
  const fetchInvoices = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://127.0.0.1:8080/api/purchases?_t=${timestamp}`,
        {
          headers: {
    ...
    try {
      ...
      if (newInvoice.id) {
        ...
        response = await axios.put(
          `http://127.0.0.1:8080/api/purchases/${newInvoice.id}`,
          invoiceData,
          {
            headers: {
              Authorization: `Bearer ${currentUser.token}`,
        ...
        response = await axios.post(
          "http://127.0.0.1:8080/api/purchases",
          invoiceData,
          {
            headers: {
              Authorization: `Bearer ${currentUser.token}`,
  ...
  const handleDeleteInvoice = async (id) => {
    ...
    try {
      setLoading(true);
      await axios.delete(`http://127.0.0.1:8080/api/purchases/${id}`, {
        headers: {
          Authorization: `Bearer ${currentUser.token}`,
          "Content-Type": "application/json",
        },
