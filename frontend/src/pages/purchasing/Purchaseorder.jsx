                    ...
                    {order.orderItems.map((item) => (
                      <li key={item.id}>
                        {(item.product?.product_name || item.description) +
                          ` (Qty: ${item.qty}, Unit Price: ${formatCurrency(item.unit_price)})`}
                      </li>
                    ))}
                  </ul>
                ) : (
...
const PurchaseOrder = () => {
  ...
  const [items, setItems] = useState([
    { productId: null, description: "", qty: 1, unitPrice: 0, total: 0 },
  ]);
  ...
  const handleItemChange = (index, e) => {
    ...
    if (name === "description") {
      // Find product by name and update description, unitPrice, and productId accordingly
      const product = products.find((p) => p.product_name === value);
      if (product) {
        updatedItems[index].description = product.product_name;
        updatedItems[index].unitPrice = product.buying_cost || 0;
        updatedItems[index].productId = product.product_id;
      } else {
      ...
      updatedItems[index][name] =
        name === "qty" || name === "unitPrice" ? Number(value) : value;
    }
