...
const getExpiryStatus = (expiryDateString) => {
  ...
  try {
    ...
    if (isNaN(expiryDate.getTime())) {
      console.error('Invalid expiry date string for comparison:', expiryDateString);
    ...
    if (dayDiff <= 30) {
      return { className: 'text-yellow-600 font-semibold', status: 'Upcoming Expired' };
    }
    return { className: 'text-green-600', status: 'Not Expired' };
  } catch (error) {
    console.error('Error processing expiry date for status:', expiryDateString, error);
...
const Expiry = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      ...
      try {
        ...
        setItems([]);
        toast.error('Error fetching items: ' + (e.response?.data?.message || e.message));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const filteredItems = useMemo(() => {
    ...
    if (refDateObj && filterOption !== 'all') {
      result = result.filter((item) => {
        ...
        try {
          ...
          console.error('Error processing item date during filtering:', item.expiry_date, e);
          return false;
        }
      });
    ...
    result.sort((a, b) => {
      const dateA = a.expiry_date ? new Date(a.expiry_date + 'T00:00:00Z').getTime() : Infinity;
      const dateB = b.expiry_date ? new Date(b.expiry_date + 'T00:00:00Z').getTime() : Infinity;
  ...
  const handlePrint = () => {
    ...
    if (filteredItems.length === 0) {
      const refDateValid = !isNaN(new Date(referenceDate + 'T00:00:00Z').getTime());
  ...
  const renderTableContent = () => {
    if (filteredItems.length === 0) {
      ...
      const refDateValid = !isNaN(new Date(referenceDate + 'T00:00:00Z').getTime());
      if (!refDateValid) {
        message = 'Invalid reference date selected. Please choose a valid date.';
      } else if (filterOption !== 'all') {
        message = 'No items match the current filter criteria based on the selected reference date.';
      }
      return (
        <tr>
    ...
    <div className="container min-h-screen p-4 mx-auto md:p-8 bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      <div className="p-4 mb-6 bg-white rounded-lg shadow-md md:flex md:items-end md:space-x-4">
        ...
        <div className="flex-grow mb-4 md:mb-0">
          <label htmlFor="filterOption" className="block text-sm font-medium text-gray-700">
