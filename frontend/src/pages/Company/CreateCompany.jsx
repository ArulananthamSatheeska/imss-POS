...
function CreateCompany() {
  ...
  const fetchCompanyDetails = async (companyName) => {
    try {
      const response = await axios.get(
        `https://sharvakshafoodcity.com.lk/backend/public/api/companies/${companyName}`
      );
      ...
      const preparedData = {
        ...
        default_payment_methods: Array.isArray(data.default_payment_methods)
          ...
          : data.default_payment_methods
            ? JSON.parse(data.default_payment_methods)
            : ["Cash"],
  ...
  const handleSubmit = async (e) => {
    ...
    Object.keys(companyInfo).forEach((key) => {
      ...
      if (key === "company_logo") {
        ...
        const value =
          companyInfo.company_type === "Other"
            ? companyInfo.custom_company_type
    ...
    const url = isEditing
      ? `https://sharvakshafoodcity.com.lk/backend/public/api/companies/${selectedCompany}`
      : "https://sharvakshafoodcity.com.lk/backend/public/api/companies";
    const method = "post";
  ...
  const handleDeleteCompany = async (companyNameToDelete) => {
    if (
      !window.confirm(
        `Are you sure you want to delete ${companyNameToDelete}? This action cannot be undone.`
      )
    ) {
      return;
    }
    ...
    try {
      const response = await axios.delete(
        `https://sharvakshafoodcity.com.lk/backend/public/api/companies/${companyNameToDelete}`
      );
      ...
      alert(
        `Failed to delete company: ${error.response?.data?.message || "Server error"}`
      );
    }
  };
    ...
    <div className="relative min-h-screen px-4 py-8 bg-gray-100 dark:bg-gray-900 sm:px-6 lg:px-8">
      <div className="max-w-6xl p-6 mx-auto bg-white rounded-lg shadow-lg dark:bg-gray-800">
        ...
        <p className="mb-6 text-center text-gray-600 dark:text-gray-400">
          Fill in the details below to create or update a company.
        </p>
        ...
        {/* FORM START */}
        <form
          className="max-w-full mx-auto bg-transparent"
        ...
        >
          ...
          <fieldset className="p-6 mb-8 rounded-lg shadow-sm bg-gray-50 dark:bg-gray-700">
            <legend className="flex items-center mb-4 text-2xl font-semibold text-gray-800 dark:text-white">
              <MapPinIcon className="w-6 h-6 mr-2 text-indigo-500" /> Address &
              Contact Details
            </legend>
          ...
          <fieldset className="p-6 mb-8 rounded-lg shadow-sm bg-gray-50 dark:bg-gray-700">
            <legend className="flex items-center mb-4 text-2xl font-semibold text-gray-800 dark:text-white">
              <CreditCardIcon className="w-6 h-6 mr-2 text-indigo-500" /> Tax &
              Financial Details
            </legend>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              <div className="flex flex-col">
          ...
          <fieldset className="p-6 mb-8 rounded-lg shadow-sm bg-gray-50 dark:bg-gray-700">
            <legend className="flex items-center mb-4 text-2xl font-semibold text-gray-800 dark:text-white">
              <UserCircleIcon className="w-6 h-6 mr-2 text-indigo-500" /> Owner
              & User Setup
            </legend>
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              <div className="flex flex-col">
              ...
              <div className="flex flex-col">
                ...
                <input
                  ...
                  className="p-3 mt-2 border border-gray-300 rounded-lg shadow-sm dark:border-gray-600 focus:border-blue-500 focus:ring focus:ring-blue-200 dark:focus:ring-blue-500 dark:bg-gray-800 dark:text-white"
                  placeholder={
                    isEditing ? "Enter new password to change" : "Required"
                  }
          ...
          <fieldset className="p-6 mb-8 rounded-lg shadow-sm bg-gray-50 dark:bg-gray-700">
            <legend className="flex items-center mb-4 text-2xl font-semibold text-gray-800 dark:text-white">
              <CogIcon className="w-6 h-6 mr-2 text-indigo-500" /> POS & System
              Settings
            </legend>
                ...
                <ul className="space-y-3">
                  {companies.map((company) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          handleDeleteCompany(company.company_name)
                        }
