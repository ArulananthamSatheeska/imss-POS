...
const CategoryForm = ({ category, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (category) {
        await axios.put(`http://127.0.0.1:8080/api/categories/${category.id}`, { name });
        onSuccess("Category updated successfully!", "success");
      } else {
        await axios.post("http://127.0.0.1:8080/api/categories", { name });
...
const CategoryManagement = () => {
  ...
  const confirmDelete = async (id) => {
    try {
      const response = await axios.delete(`http://127.0.0.1:8080/api/categories/${id}`);
