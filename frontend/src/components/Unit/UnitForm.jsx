...
const UnitForm = () => {
  ...
  const fetchUnits = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:8080/api/units");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedUnit) {
        await axios.put(`http://127.0.0.1:8080/api/units/${selectedUnit.id}`, {
          unit_name: unitName,
        });
        setNotification({ message: "Unit updated successfully!", type: "success", visible: true });
        fetchUnits();
      } else {
        await axios.post("http://127.0.0.1:8080/api/units", {
          unit_name: unitName,
        });
  ...
  const confirmDelete = async (id) => {
    try {
      await axios.delete(`http://127.0.0.1:8080/api/units/${id}`);
