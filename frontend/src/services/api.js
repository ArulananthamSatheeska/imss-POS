...
const API_URL = "https://sharvakshafoodcity.com.lk/backend/public/api";
...
const requestWithRetry = async (api, requestFn, endpoint, data = null) => {
  ...
  while (retryCount < MAX_RETRIES) {
    try {
      ...
      if (retryCount < MAX_RETRIES) {
        console.warn(
          `Retrying request (${retryCount}/${MAX_RETRIES}) to ${endpoint}`
        );
  ...
  console.error(
    `API request failed after ${retryCount} retries to ${endpoint}:`,
    lastError
  );
  throw lastError;
};
...
export const createApiClient = (refreshToken) => {
  ...
  return {
    ...
    postData: async (endpoint, data) =>
      requestWithRetry(api, api.post, endpoint, data),
    putData: async (endpoint, data) =>
      requestWithRetry(api, api.put, endpoint, data),
